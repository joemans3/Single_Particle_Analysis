...
class extract_mask_properties:
    ...
    def __dict__(self):
        return {
            ...
            "longest_axis": self.longest_axis,
            "shortest_axis": self.shortest_axis,
...
if __name__ == "__main__":
    ...
    ax[1].add_patch(
        plt.Rectangle(
            (prop_obj.bounding_box[1], prop_obj.bounding_box[0]),
            prop_obj.bounding_box[3] - prop_obj.bounding_box[1],
            prop_obj.bounding_box[2] - prop_obj.bounding_box[0],
            fill=False,
            color="r",
        )
    )
    ax[1].scatter(
        prop_obj.centroid[1], prop_obj.centroid[0], color="r", label="centroid"
    )
    # plot the r_offset
    ax[1].scatter(
        prop_obj.r_offset[1], prop_obj.r_offset[0], color="g", label="r_offset"
    )
    ax[1].legend()
    plt.show()
