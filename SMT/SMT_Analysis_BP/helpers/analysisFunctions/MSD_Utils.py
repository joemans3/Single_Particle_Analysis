...
from SMT.SMT_Analysis_BP.databases import trajectory_analysis_script as tas
...
class MSD_Calculations(Calculation_abc):
    def __init__(
        ...
        **kwargs,
        ...
        self.MSD_dataset = DatabaseOrderUtil(
            data_set_RA=data_set_RA,
            pixel_to_um=pixel_to_um,
            frame_to_seconds=frame_to_seconds,
        )
    ...
    def _build_MSD_Tracks_combined(self, **kwargs):
        ...
        for i in tas.TRACK_TYPES:
            ...
            # build the MSD_Tracks
            MSD_Tracks_i, _ = MSD_Tracks(
                track_dict_bulk_temper,
                conversion_factor=self.MSD_dataset.pixel_to_um,
                tau_conversion_factor=self.MSD_dataset.frame_to_seconds,
                **kwargs,
            )
            ensemble_MSD, ensemble_MSD_error, track_MSD, track_MSD_error = MSD_Tracks_i[
                "msd_curves"
            ]
    ...
    def _build_MSD_Tracks_individual(self, **kwargs):
        ...
        for k, l in enumerate(self.MSD_dataset.data_set_names):
            for i in tas.TRACK_TYPES:
                ...
                ensemble_MSD, ensemble_MSD_error, track_MSD, track_MSD_error = (
                    MSD_Tracks_i["msd_curves"]
                )
                ensemble_displacements, track_displacements = MSD_Tracks_i[
                    "displacements"
                ]
    ...
    def combined_store(self):
        if hasattr(self, "_combined_store"):
            ...
            print(
                "You should not be here. I have built the MSD calculations for you but you should be running that before accessing the combined_store"
            )
            return self._combined_store
    ...
    def individual_store(self):
        if hasattr(self, "_individual_store"):
            ...
            self._build_MSD_Tracks()
            print(
                "You should not be here. I have built the MSD calculations for you but you should be running that before accessing the individual_store"
            )
            return self._individual_store
