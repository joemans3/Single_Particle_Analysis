...
def gaussian2D_theta(
    x, y, cen_x, cen_y, sig_x, sig_y, theta, offset, height, kwargs={}
):
...
class SM_Localization_Image:
    ...
    def _find_candidate_peaks(self, **kwargs):
        ...
        """
        find_peaks_kwargs_default = {
            "min_distance": 2,
            "threshold_abs": np.mean(self.image) + np.std(self.image) * 7.0,
            "exclude_border": 5,
        }
    ...
    def _fit_peaks(
        ...
        for peak in self.candidate_peaks:
            ...
            fit_results.append(
                self._fit_model_to_roi(
                    roi,
                    peak,
                    fit_model_residual,
                    fit_model_params,
                    fit_model_params_bounds,
                )
            )
            ...
            if self.plot_fit_results:
                ...
                ax.plot_wireframe(
                    *self._get_roi_coordinates(roi, peak), roi, color="green"
                )
                # plot the fit results using a wireplot with the gaussian model
                ax.plot_wireframe(
                    *self._get_roi_coordinates(roi, peak),
                    gaussian2D(
                        *self._get_roi_coordinates(roi, peak),
                        **fit_results[-1].params.valuesdict(),
                    ),
                    color="red",
                )
    ...
    def _get_roi_around_peak(self, peak: np.ndarray, roi_size: int = 5):
        ...
        roi = self.image[
            peak[0] - roi_size : peak[0] + roi_size + 1,
            peak[1] - roi_size : peak[1] + roi_size + 1,
        ]
        return roi

    def _fit_model_to_roi(
        self,
        roi: np.ndarray,
        peak: np.ndarray,
        fit_model_residual: callable,
    ...
    def _get_model_params(self, fit_model_params: dict, fit_model_params_bounds: dict):
        ...
        for key in fit_model_params.keys():
            ...
            params.add(
                key,
                value=fit_model_params[key],
                min=fit_model_params_bounds[key][0],
                max=fit_model_params_bounds[key][1],
            )
        return params
    ...
    def _get_roi_coordinates(self, roi: np.ndarray, peak: np.ndarray):
        ...
        x, y = np.meshgrid(
            np.arange(peak[0] - roi.shape[0] // 2, peak[0] + roi.shape[0] // 2 + 1),
            np.arange(peak[1] - roi.shape[1] // 2, peak[1] + roi.shape[1] // 2 + 1),
        )
        return x, y

    def _fit_model_to_data(
        self,
        fit_model_residual: callable,
        params: Parameters,
        x: np.ndarray,
    ...
    def _convert_fit_result_pandas(self) -> pd.DataFrame:
        ...
        fit_results_df = pd.DataFrame(
            columns=list(self.fit_parameters["fit_model_params"].keys())
            + ["Image_Background_Mean", "Image_Background_Std"]
        )
        ...
        for result in self.fit_results:
            ...
            errors = {
                key + "_err": result.params[key].stderr
            ...
            params.update(
                {
                    "Image_Background_Mean": self.img_background_mean,
                    "Image_Background_Std": self.img_background_std,
                }
            )
            # add the parameters to the dataframe using concat
            fit_results_df = pd.concat(
                [fit_results_df, pd.DataFrame(params, index=[0])], ignore_index=True
            )
    ...
    def _prune_fit_results(self, params_to_prune: list, params_thresholds: list):
        ...
        for result in self.fit_results:
            ...
            if all(
                [
                    params_to_prune_values[i] >= params_thresholds[i][0]
                    and params_to_prune_values[i] <= params_thresholds[i][1]
                    for i in range(len(params_to_prune_values))
                ]
            ):
    ...
    def fit(
        ...
        **kwargs,
        ...
        self.fit_parameters = {
            "fit_model_residual": fit_model_residual,
            "fit_model_params": fit_model_params,
            "fit_model_params_bounds": fit_model_params_bounds,
        }
        ...
        if prune_direction is not None:
            self._prune_fit_results(
                list(prune_direction.keys()), list(prune_direction.values())
            )
...
class SM_localization_movie:
    ...
    def _convert_fit_result_pandas(self) -> pd.DataFrame:
        ...
        fit_results_df = pd.DataFrame(
            columns=list(self.fit_parameters["fit_model_params"].keys())
            + ["Image_Background_Mean", "Image_Background_Std", "Frame", "Spot_ID"]
        )
        ...
        for frame_num, frame_fit_results in enumerate(self.fit_results):
            ...
            for peak_num, result in enumerate(frame_fit_results):
                ...
                params.update(
                    {
                        "Image_Background_Mean": self.movie_background_mean[frame_num],
                        "Image_Background_Std": self.movie_background_std[frame_num],
                    }
                )
    ...
    def fit(
        ...
        fit_model_params_bounds: dict,
        prune_direction: dict = None,
        candidate_peaks_per_frame: List[np.ndarray] = None,
        **kwargs,
    ):
        """
        Finds candidate peaks in the image and fits them to the passed model, supports only gaussian models for now
        ...
        if candidate_peaks_per_frame is not None:
            for indx, frame in enumerate(self.movie):
                ...
                sm_localization_image = SM_Localization_Image(
                    frame, candidate_peaks_per_frame[indx]
                )
                # fit the model to the frame
                self._fit_results.append(
                    sm_localization_image.fit(
                        fit_model_residual,
                        fit_model_params,
                        fit_model_params_bounds,
                        prune_direction,
        ...
        print(
            "Time to convert fit results to pandas dataframe: {} seconds".format(
...
if __name__ == "__main__":
    ...
    # save the dataframe
    sm_localization_movie._save_dataframe(
        "/Volumes/Baljyot_HD/SMT_Olympus/Baljyot_temp/20190527/rpoc_ez/Movie/rpoc_ez_6_fit_results.csv"
    )
