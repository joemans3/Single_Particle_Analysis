...
class movie_mask_directory_structure_manager:
    ...
    def _find_mask_files(self) -> None:
        ...
        base_files = sorted_alphanumeric(
            glob.glob(os.path.join(self.full_path_to_mask_dir, "*.tif"))
        )
        if len(base_files) != 0:
            _reorder_file_names(base_files)
        self._base_files = sorted_alphanumeric(
            glob.glob(os.path.join(self.full_path_to_mask_dir, "*.tif"))
        )
        mask_files = sorted_alphanumeric(
            glob.glob(os.path.join(self.full_path_to_mask_dir, "*.npy"))
        )
        # check if there are any mask files
        if len(mask_files) != 0:
            _reorder_file_names(mask_files, end_string="_seg")
        self._mask_files = sorted_alphanumeric(
            glob.glob(os.path.join(self.full_path_to_mask_dir, "*.npy"))
        )
    ...
    def _make_directory_structure(self) -> None:
        ...
        for movie in range(len(self.mask_files)):
            ...
            path_structure["movie_dir"]["movies"][movie + 1]["mask_file_path"] = (
                self.mask_files[movie]
            )
            path_structure["movie_dir"]["movies"][movie + 1]["path"] = os.path.join(
                path_structure["movie_dir"]["Path"], "Movie_{}".format(movie + 1)
            )
            # make the movie directory
            if not os.path.isdir(
                path_structure["movie_dir"]["movies"][movie + 1]["path"]
            ):
            ...
            path_structure["movie_dir"]["movies"][movie + 1]["num_cells"] = len(
                cell_masks
            )
            ...
            # save the ROI file
            io.save_rois(
                read_npy_file(self.mask_files[movie])["masks"],
                os.path.join(
                    path_structure["movie_dir"]["movies"][movie + 1]["path"],
                    "Movie_{}".format(movie + 1),
                ),
            )
        ...
        if end_string is None:
            os.rename(
                ...
                os.path.join(
                    ...
                    base_name + "_" + str(i + 1) + file_extension,
                ),
            )
        else:
            os.rename(
                ...
                os.path.join(
                    os.path.dirname(file_path),
                    base_name + "_" + str(i + 1) + end_string + file_extension,
                ),
            )
