...
sorted_a_files = sorted(
    filez, key=lambda x: int(x.split("/")[-1].split(".")[0].split("_")[-1])
)
...
for i in range(len(sorted_a_files)):
    ...
    for j in np.arange(STACK_STEP):
        ...
        hold_img.append(
            SEGMENT_TYPE["mean"](
                temp_del[int(j * length) : int((j + 1) * length)], axis=0
            )
        )
        hold_name.append(
            os.path.join(
                root_dir,
                "Segmented_mean",
                str(int(j + 1)) + "_" + updated_movie_name + "_seg.tif",
            )
        )
