...
class run_analysis:
    ...
    def __init__(self, wd, t_string, sim=False, unique_name=""):
        ...
        self.type_of_blob = (
            "Scale"  # takes values indicated in SEGMENTATION_FOLDER_TYPES
        )

        self.blob_parameters = {
            "threshold": self.threshold_blob,
    ...
    def run_flow_sim(
        self, cd, t_string
    ):  # very hacky to get this to work for simulation data. Assumes the whole movie is one cell.
        all_files = sorted(
            glob.glob(os.path.join(cd, "Analysis", t_string + ".tif_spots.csv"))
        )
        print(all_files)
        ...
        for pp in range(len(all_files)):
            ...
            self.Movie[str(pp)].Cells[str(0)] = Cell(
                Cell_ID=0,
                Cell_Movie_Name=0,
                bounding_box=0,
                r_offset=0,
            ...
            for j in range(len(drop_s)):
                for k in range(
                    len(drop_s[j][self.type_of_blob])
                    ...
                    self.Movie[str(pp)].Cells[str(0)].All_Drop_Collection[
                        str(j) + "," + str(k)
                    ] = drop_s[j][self.type_of_blob][k]
                    self.Movie[str(pp)].Cells[str(0)].All_Drop_Verbose[
                        str(j) + "," + str(k)
                    ] = {
                        "Fitted": drop_s[j]["Fitted"][k],
    ...
    def _read_track_data_nocells(self, wd, t_string, **kwargs):
        ...
        analysis_path = os.path.join(
            self._Analysis_path_util(), t_string + "_**.tif_spots.csv"
        )
        ...
        for pp in range(len(all_files)):
            ...
            if self.a_file_style == "new":
                ...
                test = test_t[
                    :, [0, 3, 1, 2, 4]
                ]  # reorder the columns to match the old style
            else:
                test = np.loadtxt(
                    "{0}".format(all_files[pp]), delimiter=","
                )  # CUSTOM SCRIPTING TRACKMATE OUTPUT STYLE
            ...
            drop_files, seg_files = self._load_segmented_image_locations(
                pp=all_files[pp],
                cd=cd,
                t_string=t_string,
                max_tag=max_tag,
                ...
                for j in range(len(drop_s)):
                    for k in range(
                        ...
                        if not (LOADING_DROP_BLOB_TYPES[self.type_of_blob]):
                            self.Movie[str(pp)].Cells[str(0)].All_Drop_Collection[
                                str(j) + "," + str(k)
                            ] = drop_s[j][self.type_of_blob][k][:-1]
                        if not (LOADING_DROP_BLOB_TYPES[self.type_of_blob]):
                            self.Movie[str(pp)].Cells[str(0)].All_Drop_Collection[
                                str(j) + "," + str(k)
                            ] = drop_s[j][self.type_of_blob][k]
    ...
    def _read_track_data(self, wd, t_string, **kwargs):
        ...
        if not os.path.exists(nucleoid_path):
            print(
                "No gfp folder found in {0}. Assuming no cell segmentation exists and running analysis without use of segmentation. \n Note this mean the whole frame is a considered one cell".format(
                    cd
                )
            )
        ...
        xy_frame_dir_names = IO_run_analysis._load_superSegger(
            cd, os.path.join("gfp", "Inverted_Images")
        )  #'/gfp/Inverted_Images')
        ...
        for pp in range(len(movies)):
            ...
            for i in range(len(movies[pp])):
                ...
                try:
                    nuc_img = import_functions.read_file(
                        self.Movie[str(pp)].Movie_nucleoid
                    )
                    padded_mask = pad_array(
                        movies[pp][i][7], np.shape(nuc_img), movies[pp][i][1]
                    )
                    # perform nucleoid segmentation
                    feature_mask, regions = self._find_nucleoid(
                        str(pp), str(i), nuc_img * padded_mask
                    )
                    # store the nucleoid area by using the 1s in the feature mask
                    self.Movie[str(pp)].Cells[str(i)].nucleoid_area = float(
                        np.sum(feature_mask[feature_mask == 1])
                    )
                except:
                    nuc_img = 0
                    padded_mask = 0
                self.Movie[str(pp)].Cells[str(i)] = Cell(
                    Cell_ID=i,
                    Cell_Movie_Name=all_files[pp],
                ...
                for j in range(len(x_points)):
                    ...
                    if poly.contains(point) or poly.touches(point):
                        self.Movie[str(pp)].Cells[str(i)].raw_tracks.append(
                            tracks[pp][j]
                        )
                # once the raw tracks are added, calculate the points_per_frame
                if len(self.Movie[str(pp)].Cells[str(i)].raw_tracks) != 0:
                    self.Movie[str(pp)].Cells[
                        str(i)
                    ].points_per_frame = points_per_frame_bulk_sort(
                        x=np.array(self.Movie[str(pp)].Cells[str(i)].raw_tracks)[:, 2],
                ...
                if not (LOADING_DROP_BLOB_TYPES[self.type_of_blob]):
                    for j in range(len(drop_s)):
                        for k in range(
                            ...
                            point = Point(
                                drop_s[j][self.type_of_blob][k][0],
                                drop_s[j][self.type_of_blob][k][1],
                            )
                            if poly.contains(point) or poly.touches(point):
                                # name the drop with j = sub-frame number (0-4), and k = unique ID for this drop in the j-th sub-frame
                                self.Movie[str(pp)].Cells[str(i)].All_Drop_Collection[
                                    str(j) + "," + str(k)
                                ] = drop_s[j][self.type_of_blob][k]
                                self.Movie[str(pp)].Cells[str(i)].All_Drop_Verbose[
                                    str(j) + "," + str(k)
                                ] = {
                                    "Fitted": drop_s[j]["Fitted"][k],
                                    "Scale": drop_s[j]["Scale"][k],
    ...
    def read_track_data_masked_method(self, wd, t_string, **kwargs):
        ...
        if len(all_files) != len(path_structure_copy.keys()):
            ValueError(
                "The number of movies in the masking directory does not match the number of movies in the analysis folder. \n Please check the directory structure."
            )
        ...
        for i in range(len(path_structure_copy.keys())):  # loop over all the movies
            ...
            if not (LOADING_DROP_BLOB_TYPES[self.type_of_blob]):
                # blob analysis
                blobs_found = self._blob_detection_utility(
                    seg_files=seg_files,
                    movie_ID=i,
                    plot=False,
                    kwargs=self.blob_parameters,
            ...
            for jj in range(len(path_structure_copy[movie_ID]["cells"])):
                cell_ID = sorted_alphanumeric(
                    list(path_structure_copy[movie_ID]["cells"].keys())
                )[jj]
                if (check_bool) and (not self.overwrite_cell_localizations):
                    # load the cell data from the directory structure
                    try:
                        tracked_data = self._load_track_data(
                            path_structure_copy[movie_ID]["cells"][cell_ID][
                                "localizations_path"
                            ],
                            skiprows=1,
                        )
                        ...
                        ValueError(
                            "Movie_ID: {0}, cell_ID {1}, does not have a localizations file. \n Please check the directory structure.".format(
                                movie_ID, cell_ID
                            )
                        )
                else:
                ...
                if len(self.Movie[movie_ID].Cells[cell_ID].raw_tracks) != 0:
                    self.Movie[movie_ID].Cells[
                        ...
                        x=np.array(self.Movie[movie_ID].Cells[cell_ID].raw_tracks)[
                            :, 2
                        ],
                        y=np.array(self.Movie[movie_ID].Cells[cell_ID].raw_tracks)[
                            :, 3
                        ],
                        t=np.array(self.Movie[movie_ID].Cells[cell_ID].raw_tracks)[
                            :, 1
                        ],
                    )
                if not (LOADING_DROP_BLOB_TYPES[self.type_of_blob]):
                        ...
                        for k in range(
                            ...
                            if poly.contains(blob_point) or poly.touches(blob_point):
                                ...
                                self.Movie[movie_ID].Cells[cell_ID].All_Drop_Collection[
                                    str(j) + "," + str(k)
                                ] = blobs_found[j][self.type_of_blob][k]
                                self.Movie[movie_ID].Cells[cell_ID].All_Drop_Verbose[
                                    str(j) + "," + str(k)
                                ] = {
                                    "Fitted": blobs_found[j]["Fitted"][k],
        ...
        for i, j in self.Movie.items():
            ...
            for k, l in j.Cells.items():
                ...
                if len(l.raw_tracks) != 0:
                    if self.sim == True:
                        sorted_track = self._convert_track_frame(
                            np.array(l.raw_tracks), order=(0, 3, 1, 2, 4)
                        )
                    else:
    ...
    def _map_TrackstoDrops(self, i, k, sorted_tracks):
        ...
        list_drop_track = [
            {} for i in range(len(self.Movie[i].Cells[k].All_Drop_Collection))
        ]
        ...
        for ii, j in self.Movie[i].Cells[k].All_Drop_Collection.items():
            ...
            for kk in range(
                len(sorted_tracks[0])
            ):  # go over all sorted tracks subframes
                if (
                    int(ii[0]) == kk
                ):  # make sure tracks are of the same subframe ID as the drop we are looping over
                    for l in range(
                        len(sorted_tracks[0][kk])
                    ):  # running over the tracks in frame k
                        ...
                        if len(sorted_tracks[0][kk][l]) >= self.blob_t_len_l:
                            ...
                            n_dist = dist(
                                np.array(sorted_tracks[1][kk][l]),
                                np.array(sorted_tracks[2][kk][l]),
                                j[0],
                                j[1],
                            )
                            percent_drop_in = float(np.sum(n_dist <= j[2])) / len(
                                n_dist
                            )  # find percentage of track loc inside
                            if (
                                percent_drop_in >= self.minimum_percent_per_drop_in
                            ):  # if condition holds
                                key = str(kk) + "," + str(l)
                                checks_low = 0
                                checks_none = 0  # need len(seg frames) to go forward and use this track
                                for list_drop in range(
                                    len(list_drop_track)
                                ):  # loop over all the drops
                                    if (
                                        key in list_drop_track[list_drop]
                                    ):  # is this track already in a drop?
                                        # is the value of the percent drop in larger that the one we find in the database?
                                        if (
                                            list_drop_track[list_drop][key]
                                            < percent_drop_in
                                        ):
                                            del list_drop_track[list_drop][
                                                key
                                            ]  # delete that mapping from the database
                                ...
                                if (checks_low == 1) or (
                                    checks_none == len(list_drop_track)
                                ):
                                    list_drop_track[len(drop_ID_list) - 1][key] = (
                                        percent_drop_in
                                    )
    ...
    def _make_TrueDrop(self, i, k, drop_ID_list, list_drop_track, sorted_tracks):
        ...
        ]  # holder to true drops per frame
        ...
        for ii, j in self.Movie[i].Cells[k].All_Drop_Collection.items():
            ...
            if len_drop_tracks >= self.minimum_tracks_per_drop:
                ...
                self.Movie[i].Cells[k].Drop_Verbose[ii] = (
                    self.Movie[i].Cells[k].All_Drop_Verbose[ii]
                )  # store verbose info for this viable drop
    ...
    def _analyse_cell_tracks_utility(self, i, k, sorted_tracks):
        ...
        list_drop_track, drop_ID_list = self._map_TrackstoDrops(
            i=i, k=k, sorted_tracks=sorted_tracks
        )
        # find the true drops
        true_drop_per_frame = self._make_TrueDrop(
            i=i,
            k=k,
            ...
            if len(true_drop_per_frame[ii]) == 0:
                for l in range(
                    ...
                    track = Trajectory(
                        ...
                        MSD_total_um=con_pix_si(
                            MSD_tavg(
                                sorted_tracks[1][ii][l],
                                sorted_tracks[2][ii][l],
                                sorted_tracks[4][ii][l],
                            ),
                            which="msd",
                        ),
                        Intensity=sorted_tracks[3][ii][l],
                    )
                    self.Movie[i].Cells[k].All_Tracjectories[str(ii) + "," + str(l)] = (
                        track
                    )
                    self.Movie[i].Cells[k].No_Drops_Trajectory_Collection[
                        str(ii) + "," + str(l)
                    ] = track

        # create the mapping for each viable drop
        for ii, j in self.Movie[i].Cells[k].Drop_Collection.items():
            self.Movie[i].Cells[k].Trajectory_Collection[ii] = Trajectory_Drop_Mapping(
                ii
            )

        for kk in range(len(sorted_tracks[0])):  # loop over the subframes
            if (
                len(true_drop_per_frame[kk]) != 0
            ):  # make sure we only consider drops in this subframe
                for l in range(
                    len(sorted_tracks[0][kk])
                ):  # loop over tracks in this subframe
                    temp_in = {}
                    temp_io = {}
                    temp_ot = {}
                    inx_in = 0
                    ...
                    inx_ot = 0
                    for ii, j in (
                        self.Movie[i].Cells[k].Drop_Collection.items()
                    ):  # loop over viable drops
                        if int(ii[0]) == kk:  # make sure its in this subframe
                            ...
                            if (
                                n_dist <= j[2]
                            ).all():  # if all localizations are inside
                                if len(temp_in) == 0:
                                    temp_in[ii] = [n_dist, percent_in]
                                    inx_in = ii
                                else:
                                    print(
                                        "IN track occurs twice! Indexs={0}".format(
                                            str(i)
                                            + ","
                                            + str(k)
                                            + ","
                            ...
                            elif (n_dist >= j[2]).all() or (
                                percent_in <= self.lower_bp
                            ):
                                if (
                                    len(temp_ot) == 0
                                ):  # if this track doesn't belong to any drop add it this drop
                                    ...
                                    for tt, tv in temp_ot.items():
                                        index_tt = tt  # find the drop it belonged to
                                    if (
                                        temp_ot[index_tt][1] < percent_in
                                    ):  # if the percent in smaller delete the entry and add this one
                                        del temp_ot[index_tt]
                                        temp_ot[ii] = [n_dist, percent_in]
                                        inx_ot = ii
                                    elif (
                                        temp_ot[index_tt][1] == percent_in
                                    ):  # if percent is same, base it on which is closer in distance
                                        if np.mean(temp_ot[index_tt][0]) > np.mean(
                                            n_dist
                                        ):
                                            del temp_ot[index_tt]
                                            temp_ot[ii] = [n_dist, percent_in]
                            ...
                            elif (percent_in > self.lower_bp) and (
                                percent_in < self.upper_bp
                            ):  # repeat for in/out
                                if len(temp_io) == 0:
                                    ...
                                    if temp_io[index_tt][1] < percent_in:
                                        ...
                                        if np.mean(temp_io[index_tt][0]) > np.mean(
                                            n_dist
                                        ):
                                            del temp_io[index_tt]
                                            temp_io[ii] = [n_dist, percent_in]
                    ...
                    if len(temp_in) == 0:
                        if len(temp_io) == 0:
                            ...
                            self.Movie[i].Cells[k].Trajectory_Collection[
                                inx_ot
                            ].OUT_Trajectory_Collection[str(kk) + "," + str(l)] = track
                            # update the All_Trajectories dictionary with a unique key if (i,k) and value of a Trajectory() object.
                            self.Movie[i].Cells[k].All_Tracjectories[
                                str(kk) + "," + str(l)
                            ] = track
                        else:
                            track = self._makeTrackCls(
                            ...
                            self.Movie[i].Cells[k].Trajectory_Collection[
                                inx_io
                            ].IO_Trajectory_Collection[str(kk) + "," + str(l)] = track
                            # update the All_Trajectories dictionary with a unique key if (i,k) and value of a Trajectory() object.
                            self.Movie[i].Cells[k].All_Tracjectories[
                                str(kk) + "," + str(l)
                            ] = track
                        ...
                        self.Movie[i].Cells[k].Trajectory_Collection[
                            inx_in
                        ].IN_Trajectory_Collection[str(kk) + "," + str(l)] = track
                        # update the All_Trajectories dictionary with a unique key if (i,k) and value of a Trajectory() object.
                        self.Movie[i].Cells[k].All_Tracjectories[
                            str(kk) + "," + str(l)
                        ] = track
    ...
    def _load_segmented_image_locations(
        self,
        pp,
        cd,
        t_string,
        ...
        min_tag,
        seg_name="TRACKMATE",
        analysis_name="TRACKMATE",
    ):
        if self.a_file_style == "old":
            if len(pp) == max_tag:
                tag = pp[
                    len(cd) + len("/Analysis/" + t_string + "_") : len(cd)
                    + len("/Analysis/" + t_string + "_")
                    + 2
                ]
            else:
                tag = pp[
            ...
            if len(pp) == max_tag:
                tag = pp[
                    len(cd) + len("/Analysis_new/" + t_string + "_") : len(cd)
                    + len("/Analysis_new/" + t_string + "_")
                    + 2
                ]
            else:
                tag = pp[
        ...
        if max_tag != min_tag:
            ...
            drop_files = sorted(
                glob.glob(
                    os.path.join(
                        cd_path_seg,
                        ANALYSIS_FOLDER_TYPES[analysis_name],
                        "*_" + t_string + "_{0}_seg.tif_spots.csv".format(tag[:]),
                    )
                )
            )
            ...
            if (
                len(
                    sorted(
                        glob.glob(
                            os.path.join(
                                cd_path_seg,
                                "*_" + t_string + "_{0}_seg.tif".format(tag[:]),
                            )
                        )
                    )
                )
                ...
                seg_files = sorted(
                    glob.glob(
                        os.path.join(cd_path_seg, "*" + "_{0}_seg.tif".format(tag[:]))
                    )
                )
            else:
                seg_files = sorted(
                    glob.glob(
                        os.path.join(
                            cd_path_seg, "*_" + t_string + "_{0}_seg.tif".format(tag[:])
                        )
                    )
                )
        else:
            ...
            drop_files = sorted(
                glob.glob(
                    os.path.join(
                        cd_path_seg,
                        ANALYSIS_FOLDER_TYPES[analysis_name],
                        "*_" + t_string + "_{0}_seg.tif_spots.csv".format(tag[0]),
                    )
                )
            )
            if (
                len(
                    sorted(
                        glob.glob(
                            os.path.join(
                                cd_path_seg,
                                "*_" + t_string + "_{0}_seg.tif".format(tag[0]),
                            )
                        )
                    )
                )
                ...
                seg_files = sorted(
                    glob.glob(
                        os.path.join(cd_path_seg, "*" + "_{0}_seg.tif".format(tag[0]))
                    )
                )
            else:
                seg_files = sorted(
                    glob.glob(
                        os.path.join(
                            cd_path_seg, "*_" + t_string + "_{0}_seg.tif".format(tag[0])
                        )
                    )
                )
    ...
    def _load_segmented_image_data(self, drop_files, use_cols=(0, 1, 2), skiprows=0):
        ...
        for i in drop_files:
            points = np.loadtxt(
                "{0}".format(i), delimiter=",", usecols=use_cols, skiprows=skiprows
            )
    ...
    def _get_frame_cell_mask(self, mask, frame, movie_ID):
        if len(self.Movie) != 0:
            if isinstance(frame, int):
                ...
                return (
                    mask * np.asarray(self.Movie[movie_ID].Movie_location)[frame]
                )  # what does this do?
        else:
            raise Exception("There are no Movies in this dataset yet.")
            ...
            for movie_dir in movie_dirs:
                ...
                if len(cell_dirs) != 0:
                    ...
                    for cell_dir in cell_dirs:
                        if (
                            len(glob.glob(os.path.join(cell_dir, "localizations.csv")))
                            == 0
                        ) or (len(glob.glob(os.path.join(cell_dir, "mask.tif"))) == 0):
                            return_type = False
                else:
                    return_type = False
        else:
    ...
    def _convert_track_frame(self, track_set, **kwargs):
        ...
        **kwargs : dict
        ...
        for i in range(len(tp)):
            u_track, utrack_ind, utrack_count = np.unique(
                tp[i], return_index=True, return_counts=True
            )
            ...
            f_t = []
            cut = u_track[
                (utrack_count >= track_len_lower) * (utrack_count <= track_len_upper)
            ]
    ...
    def _convert_track_data_for_cell(
        self, path, path_structure, cell_ID, movie_ID
    ) -> None:
        """
        ...
        tracked_data_copy = pd.DataFrame(
            tracked_data_copy, columns=tracked_data.columns
        )
        # now we have the filtered data, now we need to save it as a csv in the cell directory
        tracked_data_copy.to_csv(
            path_structure[movie_ID]["cells"][cell_ID]["localizations_path"],
            index=False,
        )
    ...
    def read_parameters(
        ...
        t_len_l=10,
        t_len_u=1000,
        MSD_avg_threshold=0.0001,
        upper_bp=0.99,
        lower_bp=0.50,
        max_track_decomp=1.0,
        conversion_p_nm=130,
        minimum_tracks_per_drop=3,
        minimum_percent_per_drop_in=1.0,
        cluster_t_len_l=7,
    ):
        """
        ...
        """
        self.pixel_to_um = conversion_p_nm / 1000.0
                ...
                for k, l in j.Cells.items():
                    for n, m in l.All_Tracjectories.items():
                        ...
                        if m.Classification == "IN":
                            track_dict_in[counting_val] = np.array(
                                [m.X, m.Y, m.Frames]
                            ).T
                        elif m.Classification == "IO":
                            track_dict_io[counting_val] = np.array(
                                [m.X, m.Y, m.Frames]
                            ).T
                        elif m.Classification == "OUT":
                            track_dict_out[counting_val] = np.array(
                                [m.X, m.Y, m.Frames]
                            ).T
        else:
            for i, j in Movie.items():
    ...
    def _store_combined_SMAUG_files(self, combined_dir_name, Movie=None):
        ...
        track_dict = self._convert_to_track_dict_bulk(
            Movie=Movie, track_name_original=False
        )
        ...
        sio.savemat(
            os.path.join(smaug_ALL_dir, unique_identifier + "_ALL.mat"),
            {"trfile": Smaug_all},
        )
        sio.savemat(
            os.path.join(smaug_IN_dir, unique_identifier + "_IN.mat"),
            {"trfile": Smaug_IN},
        )
        sio.savemat(
            os.path.join(smaug_OUT_dir, unique_identifier + "_OUT.mat"),
            {"trfile": Smaug_OUT},
        )
        sio.savemat(
            os.path.join(smaug_IO_dir, unique_identifier + "_IO.mat"),
            {"trfile": Smaug_IO},
        )
    ...
    def _make_SMAUG_files(self, Movie=None, dir_name="SMAUG") -> None:
        ...
        for i, j in Movie.items():
            ...
            unique_identifier = (
                str(self.my_name) + "_" + str(self.t_string) + "_" + str(i)
            )
            # save the files
            sio.savemat(
                os.path.join(smaug_ALL_dir, unique_identifier + "_ALL.mat"),
            ...
            sio.savemat(
                os.path.join(smaug_NONE_dir, unique_identifier + "_NONE.mat"),
                {"trfile": Smaug_NONE},
            )
    ...
    def _store_combined_NOBIAS_files(self, combined_dir_name, Movie=None):
        ...
        unique_identifier = str(self.my_name) + "_" + str(self.t_string)
        sio.savemat(
            os.path.join(nobias_ALL_dir, unique_identifier + "_ALL.mat"), NOBIAS_all
        )
        sio.savemat(
            os.path.join(nobias_IN_dir, unique_identifier + "_IN.mat"), NOBIAS_IN
        )
        sio.savemat(
            os.path.join(nobias_OUT_dir, unique_identifier + "_OUT.mat"), NOBIAS_OUT
        )
        sio.savemat(
            os.path.join(nobias_IO_dir, unique_identifier + "_IO.mat"), NOBIAS_IO
        )
    ...
    def _make_NOBIAS_files(self, Movie=None, taus=1, dir_name="NOBIAS") -> None:
        ...
        tracks = self._convert_to_track_dict_bulk(
            Movie=Movie, track_name_original=False
        )
        ...
        # save the files
        sio.savemat(
            os.path.join(track_IN_dir, unique_identifier + "_IN.mat"),
            track_IN[taus - 1],
        )
        sio.savemat(
            os.path.join(track_OUT_dir, unique_identifier + "_OUT.mat"),
            track_OUT[taus - 1],
        )
        sio.savemat(
            os.path.join(track_IO_dir, unique_identifier + "_IO.mat"),
            track_IO[taus - 1],
        )
        sio.savemat(
            os.path.join(track_ALL_dir, unique_identifier + "_ALL.mat"),
            track_ALL[taus - 1],
        )
    ...
    def _find_nucleoid(self, movie_ID, cell_ID, img=0):
        ...
        if isinstance(img, int):
            img = (
                import_functions.read_file(self.Movie[movie_ID].Movie_nucleoid)
                * self.Movie[movie_ID].Cells[cell_ID].Cell_Nucleoid_Mask
            )
    ...
    def _set_nucleoid_area_bulk(self):
        ...
        for movie in self.Movie:
            for cell in self.Movie[movie].Cells:
                ...
                self.Movie[movie].Cells[cell].nucleoid_area = float(
                    np.sum(feature_mask[feature_mask == 1])
                )
    ...
    def type_of_blob(self, value):
        if value not in SEGMENTATION_FOLDER_TYPES.keys():
            raise ValueError(
                "The type of blob must be one of the following: {0}".format(
                    SEGMENTATION_FOLDER_TYPES.keys()
                )
            )
        else:
    ...
    def track_durations(self) -> dict:
        ...
        if not hasattr(self, "_track_durations"):
            ...
            self._track_durations, self.track_collection = (
                self._track_collection_utility()
            )
...
class Cell:
    ...
    def points_per_frame(self, points_per_frame: dict) -> None:
        ...
        self.area_of_points_per_frame = area_points_per_frame(
            self._points_per_frame, radius_of_gyration
        )
    ...
    def density_per_frame(self) -> dict:
        ...
        self._density_per_frame = {
            frame: len(self.points_per_frame[frame])
            / self.area_of_points_per_frame[frame]
            for frame in self.points_per_frame.keys()
        }
...
class boundary_analysis:
    ...
    def _xy_angle_from_drop(collection_traj, cell_obj):
        ...
        for i, k in collection_traj.items():
            ...
            drop_center_dist = (
                dist(x_val, y_val, drop_data[0], drop_data[1])
            ) / drop_data[2]
    ...
    def _directional_variableTracks(x, y, drops):
        ...
        angles, dist_center, directional_displacements = (
            boundary_analysis._directional_displacement_utility(mapped)
        )
    ...
    def _directional_displacement_utility(obj):
        ...
        for i in obj:
            ...
            angles += list(angle_multi(v1))[:-1]
            directional = con_pix_si(
                np.diff(dist(i[0], i[1], i[2][0], i[2][1])), which="um"
            )
    ...
    def directional_displacement(collection_traj, cell_obj):
        ...
        for i, k in collection_traj.items():
            ...
            directional = con_pix_si(
                np.diff(dist(x_val, y_val, drop_data[0], drop_data[1])), which="um"
            )
            directional_displacement += list(directional)
            dist_center += list(drop_center_dist)[:-1]
        return [angles, dist_center, directional_displacement]
            ...
            for n, m in i.Cells.items():  # cells
                for o, p in m.Trajectory_Collection.items():  # drop trajectory mapping
                    if kwargs.get("IN", False) == True:
                        angle, dist_center, dir_displacement = (
                            self.directional_displacement(p.IN_Trajectory_Collection, m)
                        )
                        angles += angle
                    ...
                    if kwargs.get("IO", False) == True:
                        angle, dist_center, dir_displacement = (
                            self.directional_displacement(p.IO_Trajectory_Collection, m)
                        )
                        angles += angle
                        displacements += dir_displacement
                        dist_centers += dist_center
                    if kwargs.get("OT", False) == True:
                        angle, dist_center, dir_displacement = (
                            self.directional_displacement(
                                p.OUT_Trajectory_Collection, m
                            )
                        )
                        angles += angle
                        displacements += dir_displacement
    ...
    def plot_directional_displacements(**kwargs):
        ...
        ax_1.errorbar(
            (_[1:] + _[:-1]) / 2, mean, yerr=std / np.sqrt(len(mean)), fmt="r-"
        )
        ax_1.set_xlabel(
            "Ralative Distance of Localization to Boundary (relative to radius)"
        )
