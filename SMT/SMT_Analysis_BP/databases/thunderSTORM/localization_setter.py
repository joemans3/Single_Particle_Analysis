...
from SMT.SMT_Analysis_BP.helpers.analysisFunctions.features_from_mask import extract_mask_properties
...
class mask_localization:
	...
	def make_loc(self):
		...
		for movie_ID in self.path_structure.keys():
			...
			movie_analysis["y [px]"] = movie_analysis["y [nm]"]/130.
			movie_analysis["x [px]"] = movie_analysis["x [nm]"]/130.
			movie_analysis["sigma [px]"] = movie_analysis["sigma [nm]"]/130.
			...
			for cell_ID in self.path_structure[movie_ID]["cells"].keys():
				...
				localizations_in_mask = pd.DataFrame(localizations_in_mask,columns=header_names)
				#save the localizations in the mask to the cell directory in the localizations.csv file
				localizations_in_mask.to_csv(self.path_structure[movie_ID]["cells"][cell_ID]["localizations_path"],index=False)
	...
	def analysis_loc_files(self):
		if hasattr(self,"_analysis_loc_files"):
			...
			analysis_loc_files = glob.glob(os.path.join(self.cd,self.analysis_folder,"*.csv"))
			#sort the files
