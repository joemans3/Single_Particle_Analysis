...
from SMT.SMT_Analysis_BP.helpers.analysisFunctions.scale_space_plus import (
    ...
    MASK_VALUE,
    RANDOM_SEED,
    SM_reconstruction_masked,
)
from SMT.SMT_Analysis_BP.helpers.clusterMethods.blob_detection import residuals_gaus2d
from SMT.SMT_Analysis_BP.helpers.clusterMethods.clustering_methods import (
...
def load_localizations(localizations_path, skiprows=4):
    ...
    df = pd.read_csv(
        ...
        skiprows=skiprows,
        names=colnames,
    )  # this can be changed depending on the format of the localizations.csv file
    return df
...
def load_localizations_TS(localizations_path, skiprows=0):
    ...
    df = pd.read_csv(
        localizations_path, delimiter=",", names=columns, skiprows=skiprows
    )  # this can be changed depending on the format of the localizations.csv file
    return df


def get_unique_localizations(
    localizations_df: pd.DataFrame, unique_loc_type: str = "first"
) -> pd.DataFrame:
    """
...
def get_unique_localizations_TS(
    localizations_df: pd.DataFrame, unique_loc_type: str = "first"
) -> pd.DataFrame:
    """
    For each unique track_ID get the first localization or the mean value of all the localizations
...
def rescale_scale_space_blob_detection(blobs, rescaling_func: callable, **kwargs):
    ...
    for i in range(len(fitted_blobs)):
        ...
        center_fitted_scaled, radius_fitted_scaled = rescaling_func(
            center_fitted, radius_fitted, type_of_convertion
        )
        center_scale_scaled, radius_scale_scaled = rescaling_func(
            center_scale, radius_scale, type_of_convertion
        )
...
class Reconstruct_Masked_PALM_DATASETS(ABC):
    ...
    def _print_message(self):
        ...
        include_all: {3}
        """.format(
            self.rescale_pixel_size, self.pixel_size, self.loc_error, self.include_all
        )
    ...
    def reconstruct(self):
        ...
        for movie_ID in path_structure_dict.keys():
            ...
            for cell_ID in path_structure_dict[movie_ID]["cells"].keys():
                ...
                localizations_path = path_structure_dict[movie_ID]["cells"][cell_ID][
                    "localizations_path"
                ]
                mask_path = path_structure_dict[movie_ID]["cells"][cell_ID]["mask_path"]
                # load the localizations
                localizations_df = self._load_localizations(
                    localizations_path=localizations_path
                )
                # get the unique localizations if include_all is False
                if self.include_all == False:
                    unique_localizations = self._unique_localization_getter(
                        localizations_df, unique_loc_type=LOCALIZATION_UNIQUE_TYPE
                    )
                ...
                localizations_df["area"] = area * ((self.pixel_size / 1000.0) ** 2)
                localizations_df.to_csv(
                    path_structure_dict[movie_ID]["cells"][cell_ID]["Analysis"][
                        "molecule_loc_path"
                    ],
                    index=False,
                )
                ...
                localizations_corrected = (
                    localizations_df[XY_NAMES].to_numpy() / CORRECTION_FACTOR
                )
                ...
                reconstruction_obj = SM_reconstruction_masked(
                    mask_img_dim, self.pixel_size, self.rescale_pixel_size
                )
                # reconstruct space
                reconstruction = reconstruction_obj.make_reconstruction(
                    localizations=localizations_corrected,
                    localization_error=loc_error_arr,
                    masked_img=mask_img,
                )
                # make the uniform reconstruction
                uniform_reconstruction = reconstruction_obj.make_uniform_reconstruction(
                    localizations=localizations_corrected,
                    localization_error=loc_error_arr,
                    masked_img=mask_img,
                )
                # now make a new object but with the original pixel size as the rescale pixel size
                reconstruction_normal_scale_obj = SM_reconstruction_masked(
                    mask_img_dim, self.pixel_size, self.pixel_size
                )
                # make the normal scale projection
                normal_scale_projection = (
                    reconstruction_normal_scale_obj.make_reconstruction(
                        localizations=localizations_corrected,
                ...
                reconstruction_obj.saving_image(
                    img=reconstruction,
                    full_path=path_structure_dict[movie_ID]["cells"][cell_ID][
                        "reconstruction_path"
                    ],
                )
                reconstruction_obj.saving_image(
                    img=uniform_reconstruction,
                    full_path=path_structure_dict[movie_ID]["cells"][cell_ID][
                        "uniform_reconstruction_path"
                    ],
                )
                reconstruction_obj.saving_image(
                    img=normal_scale_projection,
                    full_path=path_structure_dict[movie_ID]["cells"][cell_ID][
                ...
                print("#" * 100)
                print(
                    "Reconstruction complete for {0}".format(
                        path_structure_dict[movie_ID]["cells"][cell_ID][
                            "localizations_path"
                        ]
                    )
                )
                ...
                blobs_rescaled = rescale_scale_space_blob_detection(
                    blobs=blobs,
                ...
                blobs_rescaled["num_localizations"] = np.zeros(
                    len(blobs_rescaled["Scale"])
                )
                blobs_rescaled["num_localizations_scale"] = np.zeros(
                    len(blobs_rescaled["Scale"])
                )
                for i in range(len(blobs_rescaled["Scale"])):
                    ...
                    for j in range(len(localizations_corrected)):
                        ...
                        if (
                            np.sqrt(
                                (
                                    localizations_corrected[j][0]
                                    - blobs_rescaled["Fitted"][i][0]
                                )
                                ** 2
                                + (
                                    localizations_corrected[j][1]
                                    - blobs_rescaled["Fitted"][i][1]
                                )
                                ** 2
                            )
                            <= blobs_rescaled["Fitted"][i][2]
                        ):
                ...
                blobs_rescaled["Fitted"] = np.hstack(
                    (
                        blobs_rescaled["Fitted"],
                        blobs_rescaled["num_localizations"].reshape(-1, 1),
                    )
                )
                blobs_rescaled["Scale"] = np.hstack(
                    (
                        blobs_rescaled["Scale"],
                        blobs_rescaled["num_localizations_scale"].reshape(-1, 1),
                    )
                )
                np.savetxt(
                    path_structure_dict[movie_ID]["cells"][cell_ID]["Analysis"][
                        "scale_space_plus_blob_fitted_path"
                    ],
                    blobs_rescaled["Fitted"],
                    delimiter=",",
                )
                np.savetxt(
                    path_structure_dict[movie_ID]["cells"][cell_ID]["Analysis"][
                        "scale_space_plus_blob_scale_path"
                    ],
                    blobs_rescaled["Scale"],
                    delimiter=",",
                )
                ...
                print("#" * 100)
                try:
                    cluster_labels, cluster_centers, cluster_radii, loc_in_cluster = (
                        perfrom_DBSCAN_Cluster(
                            localizations=localizations_corrected,
                    ...
                    print(
                        "DBSCAN clustering failed for {0}".format(
                            path_structure_dict[movie_ID]["cells"][cell_ID][
                                "localizations_path"
                            ]
                        )
                ...
                np.savetxt(
                    path_structure_dict[movie_ID]["cells"][cell_ID]["Analysis"][
                        "DBSCAN_clusters_path"
                    ],
                    np.hstack(
                        (
                            cluster_centers,
                            cluster_radii.reshape(-1, 1),
                            loc_in_cluster.reshape(-1, 1),
                        )
                    ),
                    delimiter=",",
                )
                # now we need to save the analysis parameters(this is the same as the reconstruction parameters)
                self._save_reconstruction_parameters(
                    full_path=path_structure_dict[movie_ID]["cells"][cell_ID][
                        "Analysis"
        ...
        print("#" * 100)
        print(
            "Reconstruction and analysis complete. Come again soon! \n ill be waiting for you (ill be sad until then)! :) "
        )
        print("#" * 100)
    ...
    def _store_parameters(self):
        try:
            ...
            fitting_parameters["radius_func"] = fitting_parameters[
                "radius_func"
            ].__name__
            fitting_parameters["residual_func"] = fitting_parameters[
                "residual_func"
            ].__name__
        except:
    ...
    def _check_directory_structure(self) -> None:
        ...
        if len(movie_dirs) == 0:
            raise ValueError(
                "There are no directories that start with Movie_ in {0}".format(
                    os.path.join(self.cd, "Movies")
                )
            )
        ...
        for movie_dir in movie_dirs:
            ...
            if len(cell_dirs) == 0:
                raise ValueError(
                    "There are no directories that start with Cell_ in {0}".format(
                        movie_dir
                    )
                )
            ...
            for cell_dir in cell_dirs:
                if len(glob.glob(os.path.join(cell_dir, "localizations.csv"))) == 0:
                    raise ValueError(
                        "There are no localizations.csv files in {0}".format(cell_dir)
                    )
                if len(glob.glob(os.path.join(cell_dir, "mask.tif"))) == 0:
                    raise ValueError(
                        "There are no mask.tif files in {0}".format(cell_dir)
                    )
    ...
    def _check_if_previous_analysis_has_occured(self) -> None:
        ...
        for movie_ID in path_structure_dict.keys():
            ...
            for cell_ID in path_structure_dict[movie_ID]["cells"].keys():
                ...
                reconstruction_parameters_path = path_structure_dict[movie_ID]["cells"][
                    cell_ID
                ]["reconstruction_parameters_path"]
...
class Reconstruct_Fixed_PALM_DATASETS(Reconstruct_Masked_PALM_DATASETS):
    def __init__(
        ...
        loc_error=30,
        include_all=True,
        localization_loader: callable = load_localizations,
        unique_localization_getter: callable = get_unique_localizations,
    ):
        super().__init__(
            cd,
    ...
    def reconstructTime(self, subsample_frequency: int = 500, total_frames: int = 5000):
        ...
        for movie_ID in path_structure_dict.keys():
            ...
            for cell_ID in path_structure_dict[movie_ID]["cells"].keys():
                ...
                subsampled_reconstruction_dir = os.path.join(
                    path_structure_dict[movie_ID]["cells"][cell_ID]["path"],
                    "subsampled_reconstruction_sampling_{0}".format(
                        subsample_frequency
                    ),
                )
                ...
                for sampler in range(len(subsample_frames)):
                    ...
                    try:
                        localizations_subsampled = localizations_df[
                            (localizations_df["frame"] >= subsample_frames[sampler])
                            & (
                                localizations_df["frame"]
                                < subsample_frames[sampler + 1]
                            )
                        ]
                    except IndexError:
                    ...
                    localizations_subsampled.to_csv(
                        os.path.join(
                            subsampled_reconstruction_dir,
                            "molecule_loc_{0}.csv".format(sampler),
                        ),
                        index=False,
                    )
                    # get the dims of the mask image
                    ...
                    # correct the x,y values of the localizations using the correction factor
                    localizations_corrected = (
                        localizations_subsampled[XY_NAMES].to_numpy()
                        / CORRECTION_FACTOR
                    )
                    # make localization errors in the same length as the localizations
                    loc_error_arr = (
                        np.ones(len(localizations_corrected)) * self.loc_error
                    )
                    # create the reconstruction object
                    reconstruction_obj = SM_reconstruction_masked(
                        mask_img_dim, self.pixel_size, self.rescale_pixel_size
                    ...
                    # make the uniform reconstruction
                    uniform_reconstruction = (
                        reconstruction_obj.make_uniform_reconstruction(
                            localizations=localizations_corrected,
                            localization_error=loc_error_arr,
                            masked_img=mask_img,
                        )
                ...
                global_normal_scale_projection = np.array(
                    global_normal_scale_projection
                )
                # save the reconstruction, uniform reconstruction and normal scale projection
                reconstruction_obj.saving_image(
                    img=global_reconstruction,
                    full_path=os.path.join(
                        subsampled_reconstruction_dir, "reconstruction.tif"
                    ),
                )
                reconstruction_obj.saving_image(
                    img=global_uniform_reconstruction,
                    full_path=os.path.join(
                        subsampled_reconstruction_dir, "uniform_reconstruction.tif"
                    ),
                )
                reconstruction_obj.saving_image(
                    img=global_normal_scale_projection,
                    full_path=os.path.join(
                        subsampled_reconstruction_dir, "normal_scale_projection.tif"
                    ),
                )
                print("#" * 100)
                print(
...
if __name__ == "__main__":
    ...
    for i in range(len(global_path)):
        ...
        sm_rec = Reconstruct_Fixed_PALM_DATASETS(
            cd=global_path[i],
            blob_parameters=blob_parameters,
            fitting_parameters=fitting_parameters,
            rescale_pixel_size=10,
