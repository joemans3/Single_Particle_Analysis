...
COL_NAMES_THUNDERSTORM = ['id', 'frame', 'x [nm]', 'y [nm]', 'sigma [nm]', 'intensity [photon]', 'offset [photon]', 'bkgstd [photon]', 'chi2', 'uncertainty [nm]']
NEW_COL_NAMES = ['id', 'frame', 'x [nm]', 'y [nm]', 'sigma [nm]', 'intensity [photon]', 'offset [photon]', 'bkgstd [photon]', 'chi2', 'uncertainty [nm]', 'merged']
...
def merge_localization(tracks:pd.DataFrame, max_dist=1, max_frame_gap=1)->pd.DataFrame:
    ...
    if not set(COL_NAMES_THUNDERSTORM).issubset(tracks.columns):
        raise ValueError('tracks must contain the following columns: ' + ', '.join(COL_NAMES_THUNDERSTORM))
    ...
    clustering = AgglomerativeClustering(n_clusters=None, distance_threshold=max_dist, linkage='single', affinity=sim_affinity).fit(coords)
    ...
    for label in set(labels):
        ...
        merged_tracks = pd.concat([merged_tracks, pd.DataFrame([[merged_id, merged_frame, merged_x, merged_y, merged_sigma, merged_intensity, merged_offset, merged_bkgstd, merged_chi2, merged_uncertainty, num_localizations]], columns=NEW_COL_NAMES)], ignore_index=True)
