...
class PickleUtil(object):
    ...
    def load(full_path_to_load:str):
        ...
        if not isinstance(pickle_loaded, dict):
            ...
            warnings.warn(f'Object loaded from {full_path_to_load} is not in the PickleUtil format. Returning the object as is.')
        ...
        if all([key in pickle_loaded.keys() for key in ['name', 'docs', 'obj']]):
            ...
            print(f'Extracted PickleUtil formatted object from {full_path_to_load} with name {pickle_loaded["name"]}')
            #return the object
            return pickle_loaded
...
class TestPickleUtil(unittest.TestCase):
    ...
    def test_save_and_load(self):
        ...
        dict_to_save = {'a':1, 'b':2}
        # save the dict
        self.pickle_util.save(path=os.getcwd(), name='my_dict', docs='This is a dict I want to save', obj=dict_to_save)
        # load the dict
