...
class ScaleSpacePlus(ABC):
    ...
    def make_reconstruction(self):
        NotImplementedError('The make_reconstruction method must be implemented in the child class')
    
    @abstractmethod
    def saving_image(self):
        NotImplementedError('The saving_image method must be implemented in the child class')
...
class SM_reconstruction_image(ScaleSpacePlus):
    ...
    def __init__(self,img_dims_normal:tuple|list,
                 ...
                 rescale_pixel_size:int=10) -> None:
    ...
    def print_state(self):
        pass 
    def make_reconstruction(self,localizations:np.ndarray,localization_error:np.ndarray|float):
        '''
        Parameters:
        -----------
...
class SM_reconstruction_masked(ScaleSpacePlus):
    def __init__(self,img_dims_normal:tuple|list,
        ...
        self.pixel_size_normal = pixel_size_normal #in nm
        self.rescale_pixel_size = rescale_pixel_size #in nm
        self._img_dims = [int(i*self.pixel_size_normal/self.rescale_pixel_size) for i in self.img_dims_normal]        
        self.normal_domain = [np.arange(0,self.img_dims[0],1.),np.arange(0,self.img_dims[1],1.)]
    ...
    def make_reconstruction(self,localizations,localization_error,masked_img):
        ...
        df_localizations[['x','y']] = self.df_localizations[['x','y']] - np.min(self.masked_domain,axis=0) + BOUNDING_BOX_PADDING

        #rescale the localizations
        df_localizations[['x','y']] = df_localizations[['x','y']]*self.pixel_size_normal/self.rescale_pixel_size

        #now make an empty image space of the masked domain using the bounding box
        small_img_space = np.zeros([int((self.bounding_box[1,0]-self.bounding_box[0,0])*self.pixel_size_normal/self.rescale_pixel_size),
                                    int((self.bounding_box[1,1]-self.bounding_box[0,1])*self.pixel_size_normal/self.rescale_pixel_size)])
        domain_small = [np.arange(0,small_img_space.shape[0],1.),np.arange(0,small_img_space.shape[1],1.)]

        #loop over the localizations
        for i in range(len(df_localizations)):
            x = df_localizations.iloc[i]['x']
            y = df_localizations.iloc[i]['y']
            loc_error_val = df_localizations.iloc[i]['localization_error']/(self.rescale_pixel_size)
            sigma_shape = np.ones(2)*loc_error_val
            ...
            y_scaled = rescale_range(y,*[0,small_img_space.shape[1]],*[0,domain_max])
            #get the gaussian
            temp_scape = get_gaussian(mu = [x_scaled,y_scaled],sigma = sigma_shape,domain = domain)
            #now we have a 20x20 matrix but we need to embed it into the orginal self.img_space
            #we can find the bottom corner of the 20x20 matrix in the self.img_space
            ...
            #now we can embed the 20x20 matrix into the self.img_space
            try:
                small_img_space[x_bottom:x_bottom+domain_max,y_bottom:y_bottom+domain_max] = small_img_space[x_bottom:x_bottom+domain_max,y_bottom:y_bottom+domain_max] + np.array(temp_scape)
            except:
                print("The localization is too close to the edge of the image space. Edge cases are not implimented yet so the localization is not included in the reconstruction.")

        return small_img_space.T
    
    def make_uniform_reconstruction(self,localizations,localization_error,masked_img):
        '''
        ...
        df_localizations = self._get_uniform_localization(masked_domain=self.masked_domain,num_localizations=num_localizations)
        #move the localizations based on the domain of the bounding box
        df_localizations[['x','y']] = df_localizations[['x','y']] - np.min(self.masked_domain,axis=0)+ BOUNDING_BOX_PADDING
        #rescale the localizations
        df_localizations[['x','y']] = df_localizations[['x','y']]*self.pixel_size_normal/self.rescale_pixel_size
    ...
    def _get_uniform_localization(self,masked_domain,num_localizations):
        ...
        choice_indx = np.random.choice(np.arange(0,len(masked_domain),1),size=num_localizations,replace=True)
    ...
    def coordinate_conversion(self,spatial_dim:np.ndarray,radius:np.ndarray,conversion_type:str):
        '''
        ...
        if conversion_type == 'RC_to_Original':
            ...
            converted_dim = spatial_dim*self.rescale_pixel_size/self.pixel_size_normal
            #add the padding
            ...
            converted_dim = converted_dim*self.pixel_size_normal/self.rescale_pixel_size
            convert_radius = radius*self.pixel_size_normal/self.rescale_pixel_size
        return converted_dim,convert_radius

    def _setup_masked_domain(self,mask_value:int,masked_img_space:np.ndarray)->None:
        '''
        ...
        domain_xy = np.array([indxes[1],indxes[0]]).T #this is to correct for the fact that img space and where invert the x,y axis
        #now we can create the masked domain
    ...
    def _setup_bounding_box(self)->None:
        ...
        if not hasattr(self,'_masked_domain'):
            raise ValueError('The masked_domain attribute does not exist. Please create it first. Or pass a masked_img_space to the _setup_masked_domain method')
