...
def bootstrap_hist(data, bins, n_bootstraps=1000, bootsize=None, bootfunc=np.mean):
    ...
    hist_samples = np.array([np.histogram(s, bins=bins,weights=np.ones_like(s)/len(s))[0] for s in boot_samples])
...
def bootstrap_mean_std(values,n_bootstraps,bootsize,log_values=False):
    ...
    for i in range(n_bootstraps):
        ...
        if log_values:
            bootstrapped_values[i] = np.mean(np.random.choice(np.log10(values),size=num_values,replace=True))
        else:
            bootstrapped_values[i] = np.mean(np.random.choice(values,size=num_values,replace=False))
...
def bootstrap_statistic(values,n_bootstraps,bootsize,log_values=False,statistic="mean"):
    ...
    if statistic == "mean":
        ...
        raise ValueError("Statistic must be one of ['mean','median','std','var','min','max']")
    return value
