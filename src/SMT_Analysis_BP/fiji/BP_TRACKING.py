...
from fiji.plugin.trackmate.tracking.jaqaman import SparseLAPTrackerFactory #trackmate developer change it from time to time, if you update find the current
...
def main():
	...
	file_tif_paths_in_dir = sorted_alphanumeric(glob.glob(os.path.join(dir_containing_tif_movies, '*.tif')))
...
def tracking_plus_save(directory_path,dir_save,file_path,file_counter):
	...
	print("Working on file: {0} in directory: {1}".format(file_name_original, directory_path))

	file_name = file_name_original[:len(MOVIE_BASE_NAME)+1] + "{0}".format(file_counter)
...
#########################################################################################################################################################################################################################################################
	...
	settings.trackerSettings['LINKING_MAX_DISTANCE'] = LINKING_PARAMETERS['LINKING_MAX_DISTANCE']
	settings.trackerSettings['GAP_CLOSING_MAX_DISTANCE'] = LINKING_PARAMETERS['GAP_CLOSING_MAX_DISTANCE']
	settings.trackerSettings['SPLITTING_MAX_DISTANCE'] = LINKING_PARAMETERS['SPLITTING_MAX_DISTANCE']
	settings.trackerSettings['MERGING_MAX_DISTANCE'] = LINKING_PARAMETERS['MERGING_MAX_DISTANCE']
	settings.trackerSettings['MAX_FRAME_GAP'] = LINKING_PARAMETERS['MAX_FRAME_GAP']
	settings.trackerSettings['ALLOW_TRACK_SPLITTING'] = LINKING_PARAMETERS['ALLOW_TRACK_SPLITTING']
	settings.trackerSettings['ALLOW_TRACK_MERGING'] = LINKING_PARAMETERS['ALLOW_TRACK_MERGING']
	...
	if FILTERING_PARAMETERS['MIN_NUMBER_SPOTS_IN_TRACK'] is not None:
		settings.addTrackFilter('NUMBER_SPOTS', FILTERING_PARAMETERS['MIN_NUMBER_SPOTS_IN_TRACK'], True)
	if FILTERING_PARAMETERS['MAX_NUMBER_SPOTS_IN_TRACK'] is not None:
		settings.addTrackFilter('NUMBER_SPOTS', FILTERING_PARAMETERS['MAX_NUMBER_SPOTS_IN_TRACK'], False)
	...
	for id in model.getTrackModel().trackIDs(True):
		...
		for spot in track:
			...
			collection_spots_per_track.append([spot_label,spot_ID,track_ID,spot_quality,spot_x,spot_y,spot_z,spot_t,spot_frame,spot_radius,spot_visibility,spot_manual_spot_color,spot_mean_intensity_ch1,spot_median_intensity_ch1,spot_min_intensity_ch1,spot_max_intensity_ch1,spot_total_intensity_ch1,spot_std_intensity_ch1,spot_contrast_ch1,spot_snr_ch1])
